...
selectCity.addEventListener('change', (e) => {
  if(e.target.value == 'los-angeles') {
    document.querySelector('.shelter-list').innerHTML = " "
    fetch('https://public.gis.lacounty.gov/public/rest/services/LACounty_Dynamic/LMS_Data_Public/MapServer/158/query?where=1%3D1&outFields=*&outSR=4326&f=json')
    .then(res => res.json())
    .then(data => {
...
selectCity.addEventListener('change', (e) => {
  if(e.target.value == 'baltimore') {
    document.querySelector('.shelter-list').innerHTML = " "
    fetch('https://opendata.baltimorecity.gov/egis/rest/services/Hosted/Homeless_Shelter/FeatureServer/0/query?where=1%3D1&outFields=*&outSR=4326&f=json')
    .then(res => res.json())
    .then(data => {
